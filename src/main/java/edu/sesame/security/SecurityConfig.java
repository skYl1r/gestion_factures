...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	static HashMap<String[], String[]> routes = new HashMap<String[], String[]>() {
		{
			...
			put(new String[] { "/commandes/nouveau", "/commandes/edit", "/commandes/add", "/commandes/update",
					"/commandes/delete" }, new String[] { "UPDATE_COMMANDES" });

			put(new String[] { "/factures/nouveau", "/factures/edit", "/factures/add", "/factures/update",
					"/factures/delete" }, new String[] { "UPDATE_FACTURES" });
			put(new String[] { "/factures/print", "/factures/preview" }, new String[] { "ROLE_PRINT_FACTURES" });

			put(new String[] { "/fournisseurs/add", "/fournisseurs/update", "/fournisseurs/delete" },
					new String[] { "UPDATE_FOURNISSEURS" });

			put(new String[] { "/produits/add", "/produits/update", "/produits/delete", "/familles/save",
					"/familles/delete", "/tva/dave", "/tva/delete" }, new String[] { "UPDATE_PRODUITS" });
		}
	};
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.jdbcAuthentication().dataSource(dataSource)
				...
				.authoritiesByUsernameQuery(
						"select username as principal, role from users_roles ur,role r where ur.username=? and ur.role_id=r.id")
	...
	protected void configure(final HttpSecurity http) throws Exception {
		...
		pages.forEach((k, v) -> {
			try {
				http.authorizeRequests().antMatchers(v).hasAnyRole(k, "ADMIN");
			} catch (Exception e) {
			}
		});
		...
		routes.forEach((k, v) -> {
			try {
				http.authorizeRequests().antMatchers(k).hasAnyRole(v[0], "ADMIN");
			} catch (Exception e) {
			}
		});
