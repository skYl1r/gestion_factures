...
public class DossierController {
	...
	@Autowired
	private IDossierMetier metierDossier;
	@Autowired
	private ICommandeMetier metierCmd;
	@Autowired
	private IFactureMetier metierFct;
	@Autowired
	...
	public String index(Model model, @RequestParam(name = "page", defaultValue = "0") Integer p,
		...
		try {
			if (!dateov.isEmpty())
				dateo = mdyFormat.parse(dateov);
		} catch (Exception e) {
		}
		try {
			if (!datefr.isEmpty())
				datef = mdyFormat.parse(datefr);
		} catch (Exception e) {
		}
		...
		if (type.equals("f")) {
			if (dateo != null) {
				if (datef != null) {
					...
					dossiers = metierDossier.getDossiersClosedByDateCreation(nom, dateo, p, s);
			} else if (datef != null) {
				dossiers = metierDossier.getDossiersByDateFermeture(nom, datef, p, s);
			} else
				dossiers = metierDossier.getDossiersClosed(nom, p, s);
		} else if (type.equals("nf")) {
			if (dateo != null) {
				dossiers = metierDossier.getDossiersNotClosedByDateCreation(nom, dateo, p, s);
			} else
				dossiers = metierDossier.getDossiersNotClosed(nom, p, s);
		} else if (dateo != null) {
			if (datef != null) {
				dossiers = metierDossier.getDossiersByDateCreationFermeture(nom, dateo, datef, p, s);
			} else
				dossiers = metierDossier.getDossiersByDateCreation(nom, dateo, p, s);
		} else if (datef != null) {
			dossiers = metierDossier.getDossiersByDateFermeture(nom, datef, p, s);
		} else
			dossiers = metierDossier.getDossiers(nom, p, s);
	...
	public String saveDossier(@Valid Dossier dos, BindingResult result, Model model) {
		...
		User u = metierUser.getUser(SecurityContextHolder.getContext().getAuthentication().getName());
		dos.setUser(u);
