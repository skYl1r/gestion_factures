...
public class StatistiqueController {
	...
	@Autowired
	private ILfMetier metierLf;

	@Autowired
	private IProduitMetier metierProduit;
	@Autowired
	private IDossierMetier metierDos;
	@Autowired
	private IFamilleMetier metierFamille;
	...
	public @ResponseBody HashMap<String, List<Statistique>> getStatistiques(
		...
		String[] ms = { "Janvier", "Fevrier", "Mars", "Avril", "May", "Juin", "Juillet", "Aout", "September", "Octob",
				"November", "December" };
	...
	private void regler2(Long numDossier, Date d1, Date d2, String p, HashMap<String, List<Statistique>> listes,
			String label) {
		Integer totalv = 0, totala = 0;
		Double totalpa = 0d, totalpv = 0d;
		if (p.isEmpty()) {
	...
	public @ResponseBody HashMap<String, Object> getGlobalStatistiques(
		...
		Double res = nCmdLast != 0
				? ((nCmdLast > nCmd) ? (-((nCmdLast - nCmd) / (double) nCmdLast) * 100)
						: (((nCmd - nCmdLast) / (double) nCmdLast) * 100))
				: 0;
		...
		res = revLast != 0
				? ((revLast > rev) ? (-((revLast - rev) / (double) revLast) * 100)
						: (((rev - revLast) / (double) revLast) * 100))
				: 0;
		...
		try {
			d_1 = sf.parse(a + "-" + (m - 2) + "-1");
			d_2 = sf.parse(a + "-" + (m - 1) + "-1");
			Double lastCro = revLast - metierFacture.getRevenues(d_1, d_2);
			Double actuCro = rev - revLast;
			res = lastCro != 0
					? ((lastCro > actuCro) ? (-((lastCro - actuCro) / (double) lastCro) * 100)
							: (((actuCro - lastCro) / (double) lastCro) * 100))
					: 0;
	...
	public @ResponseBody Collection getStocks(@RequestParam(name = "m", defaultValue = "0") Integer m,
		...
		try {
			...
			if (m > 0) {
				if (j > 0)
					stocks = metierStk.getMoyenStocksJour(refProduit, a, m, j);
				else
					stocks = metierStk.getMoyenStocksMois(refProduit, a, m);
			} else
				stocks = metierStk.getMoyenStocksAnne(refProduit, a);
			...
			if (m > 0 && j > 0)
				for (int i = 0; i <= 24; i++) {
					if (list.containsKey("" + i))
						...
						res.add(new Statistique(i + ":00", "0"));
				}
			else if (m > 0)
				for (int i = 1; i <= 31; i++) {
					if (list.containsKey("" + i))
				...
				for (int i = 1; i <= 12; i++) {
					if (list.containsKey("" + i))
						...
						res.add(new Statistique("" + ms[i - 1], "0"));
				}
	...
	public @ResponseBody FusionChart getAllStocks(@RequestParam(name = "m", defaultValue = "0") Integer m,
			@RequestParam(name = "a", defaultValue = "2018") Integer a,
			@RequestParam(name = "j", defaultValue = "0") Integer j) {
		...
		try {
			...
			if (m > 0) {
				if (j > 0)
					stocks = metierStk.getMoyenStocksJour(a, m, j);
				else
					stocks = metierStk.getMoyenStocksMois(a, m);
			} else
				stocks = metierStk.getMoyenStocksAnne(a);
			...
			if (m > 0) {
				if (j > 0)
					...
					n = 31;
				for (int k = 1; k <= n; k++)
					categories.add(new Label("" + k));
			} else {
				String[] ms = { "Janvier", "Fevrier", "Mars", "Avril", "May", "Juin", "Juillet", "Aout", "September",
						"Octob", "November", "December" };
				for (int k = 0; k < n; k++)
					categories.add(new Label(ms[k]));
			}
			...
			for (i = 0; i < stocks.size(); i++) {
				...
				try {
					v = Integer.valueOf(stock[1].toString());
				} catch (Exception e) {
					v = -1;
				}
				...
				for (k = 1; k <= n; k++) {
					if (k == v) {
						...
						list.get("" + stock[0]).put(k, new Value("0"));
				}
			}
			if (i > 0)
				ret.add(new DataSet(stocks.get(i - 1)[0] + "", list.get(stocks.get(i - 1)[0]).values()));
			...
			System.err.println("stock 2 " + e.getMessage());
		}
	...
	public class Statistique {
		...
		public Statistique(String l, String v) {
			label = l;
			value = v;
		}
	}
	...
	public class DataSet {
		...
		public DataSet(String s, Collection c) {
			seriesname = s;
			data = c;
		}
	}
	...
	public class Categorie {
		...
		public Categorie(Collection<Label> c) {
			category = c;
		}
	}
	...
	public class FusionChart {
		...
		public FusionChart(Collection<Categorie> c, Collection<DataSet> d) {
			categories = c;
			dataset = d;
		}
	}
