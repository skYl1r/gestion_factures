...
public class CommandeController {
	...
	@Autowired
	private ILcMetier metierLigneCommande;
	@Autowired
	...
	public String index(Model model, @RequestParam(name = "pagevente", defaultValue = "0") Integer pv,
		...
		if (!num.isEmpty()) {
			...
			if (cmd != null) {
				if (((Dossier) session.getAttribute("dossier")).getNumero() != cmd.getDossier().getNumero())
					...
					if (cmd.getFournisseur() != null)
						model.addAttribute("commandesAchat", new Commande[] { cmd });
			...
			switch (type) {
			case "vente":
				indexVente(model, pv, s, datevente, clt, validevente);
				break;
			case "achat":
		...
		if (!frs.isEmpty()) {
			if (valide != null) {
				if (date != null)
					...
					cmds = metierCommande.getCommandesOfFournisseur(numDossier, frs, valide, pa, s);
			} else if (date != null)
				cmds = metierCommande.getCommandesOfFournisseur(numDossier, frs, date, pa, s);
			else
				cmds = metierCommande.getCommandesOfFournisseur(numDossier, frs, pa, s);
		} else if (valide != null) {
			if (date != null)
				cmds = metierCommande.getCommandesFournisseurs(numDossier, valide, date, pa, s);
			else
				cmds = metierCommande.getCommandesFournisseurs(numDossier, valide, pa, s);
		} else if (date != null)
			cmds = metierCommande.getCommandesFournisseurs(numDossier, date, pa, s);
		else
			cmds = metierCommande.getCommandesFournisseurs(numDossier, pa, s);

		double totalAchats = 0;
		for (Commande cmd : cmds)
			totalAchats += cmd.getTotal();
		...
		if (!clt.isEmpty()) {
			if (valide != null) {
				if (date != null)
					...
					cmds = metierCommande.getCommandesOfClient(numDossier, clt, valide, pv, s);
			} else if (date != null)
				cmds = metierCommande.getCommandesOfClient(numDossier, clt, date, pv, s);
			else
				cmds = metierCommande.getCommandesOfClient(numDossier, clt, pv, s);
		} else if (valide != null) {
			if (date != null)
				cmds = metierCommande.getCommandesClients(numDossier, valide, date, pv, s);
			else
				cmds = metierCommande.getCommandesClients(numDossier, valide, pv, s);
		} else if (date != null)
			cmds = metierCommande.getCommandesClients(numDossier, date, pv, s);
		else
			cmds = metierCommande.getCommandesClients(numDossier, pv, s);

		double totalVentes = 0;
		for (Commande cmd : cmds)
			totalVentes += cmd.getTotal();
	...
	public String nouveaucommande(Model model) {
		...
		if (lcs == null)
			lcs = new HashMap<String, LigneCommande>();
	...
	public String addCommande(@Valid Commande commande, BindingResult result, Model model) {
		...
		if ((commande.getClient() == null && commande.getFournisseur() == null)
				|| (commande.getClient() != null && commande.getFournisseur() != null))
	...
	public String updateCommande(Model model, @RequestParam(name = "numero", defaultValue = "0") Long numero,
		...
		try {
			...
			SimpleDateFormat mdyFormat = new SimpleDateFormat("yyyy-MM-dd");
			try {
				dateCommande = mdyFormat.parse(dateCommandeString);
			} catch (Exception e) {
				throw new Exception("Entrer une date valide !");
			}

			Boolean valide = null;
			try {
				valide = Boolean.valueOf(valideString);
			} catch (Exception e) {
				throw new Exception("Selectioner la validité du commande !");
			}
	...
	public String addlignecommande(Model model, @RequestParam(name = "produit", defaultValue = "0") String ref,
			@RequestParam(name = "commande", defaultValue = "0") Long numero,
			@RequestParam(name = "qte", defaultValue = "0") Integer qte) {
		try {
			if (qte <= 0)
				throw new Exception("Quantité minimale est 1");
			...
			if (lc == null) {
				...
				if (lc.getProduit() == null)
					throw new Exception("Selectionner un produit !");

				model.addAttribute("addLcOk", "Produit " + lc.getProduit().getDesignation() + " est ajouté");
			} else
				model.addAttribute("updateLcOk",
						"La quantité du produit " + lc.getProduit().getDesignation() + " est mise à jour à " + qte);
	...
	public String updatelignecommande(Model model, @RequestParam(name = "produit", defaultValue = "0") String ref,
			@RequestParam(name = "commande", defaultValue = "0") Long numero,
	...
	public String deletelignecommande(Model model, @RequestParam(name = "id", defaultValue = "0") Long id,
			@RequestParam(name = "numCommande", defaultValue = "0") Long numCommande) {
	...
	public @ResponseBody Result storelignecommande(
			...
			@RequestParam(name = "qte", defaultValue = "0") Integer qte) {
		if (qte <= 0)
			return new Result("La quantité doit etre au minimum 1 !", null, "");
		if (refProduit.equals("0"))
			return new Result("Selectionner d'abord le produit !", qte + "", null);
	...
	class Result implements java.io.Serializable {
		...
		Result(String m, String q, String p) {
			qte = q;
			message = m;
			produit = p;
		}
	...
	Commande validerCommande(Model model, String dateCommandeString, String valideString, String codeClient,
