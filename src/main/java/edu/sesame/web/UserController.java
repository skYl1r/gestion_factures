...
public class UserController {
	...
	@Autowired
	private UserRolesRepository urlRep;
	@Autowired
	...
	public @ResponseBody String[] updaterole(@RequestParam(name = "id") Long id,
			@RequestParam(name = "newrole") String nr) {
		if (nr.length() <= 3)
			return new String[] { "Le role doit etre au moins 4c." };
		if (rolRep.findByRole(nr) != null)
			return new String[] { "Le role déja exist !" };
		Role r = rolRep.findByIdRole(id);
		if (r == null)
			return new String[] { "Le role n'existe pas !" };
		r.setRole(nr);
		rolRep.save(r);
		return new String[] {};
	}
	...
	public String updateRole(Model m, @RequestParam("id") Long id, @RequestParam("name") String name,
			@RequestParam("des") String des) {
		Role r = rolRep.findByIdAndName(id, name);
		if (r == null)
			m.addAttribute("updateFailed", "Le role n'existe pas !");
		if (des.length() <= 2)
