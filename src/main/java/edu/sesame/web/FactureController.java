...
import edu.sesame.entities.Facture;
...
import edu.sesame.imetier.IFactureMetier;
...
public class FactureController {
	...
	@Autowired
	private IFactureMetier metierFacture;
	@Autowired
	private ICommandeMetier metierCommande;

	@Autowired
	private IProduitMetier metierProduit;
	@Autowired
	private IFournisseurMetier metierFournisseur;
	@Autowired
	private IClientMetier metierClient;
	@Autowired
	private IReglementMetier metierReglement;
	@Autowired
	private IReductionMetier metierRed;
	@Autowired
	private ILfMetier metierLigneFacture;
	@Autowired
	private IRGFMetier metierRgf;
	@Autowired
	private IRDFMetier metierRdf;
	@Autowired
	private ILivraisonMetier metierLiv;

	@Autowired
	private IStockMetier metierStk;
	...
	public ResponseEntity<InputStreamResource> index(Model model,
	...
	public String index(Model model, @RequestParam(name = "pagevente", defaultValue = "0") Integer pv,
		...
		if (num != 0) {
			...
			if (fct != null) {
				// if( ((Dossier)session.getAttribute("dossier")).getNumero() !=
				// fct.getDossier().getNumero() );
				// else
				{
			...
			switch (type) {
			case "vente":
				indexVente(model, pv, s, datevente, clt);
				break;
			case "achat":
	...
	private void indexAchat(Model model, Integer pa, Integer s, String dateachat, String frs) {
		...
		if (!dateachat.isEmpty()) {
			...
			try {
				date = mdyFormat.parse(dateachat);
			} catch (Exception e) {
				date = null;
			}
		}
		...
		if (!frs.isEmpty()) {
			if (date != null)
				fcts = metierFacture.getFacturesOfFournisseur(numDossier, frs, date, pa, s);
			else
				fcts = metierFacture.getFacturesOfFournisseur(numDossier, frs, pa, s);
		} else if (date != null)
			fcts = metierFacture.getFacturesFournisseurs(numDossier, date, pa, s);
		else
			fcts = metierFacture.getFacturesFournisseurs(numDossier, pa, s);

		double totalAchats = 0;
		for (Facture fct : fcts)
			totalAchats += fct.getTotal();
	...
	private void indexVente(Model model, Integer pv, Integer s, String datevente, String clt) {
		...
		if (!datevente.isEmpty()) {
			SimpleDateFormat mdyFormat = new SimpleDateFormat("yyyy-MM-dd");
			try {
				date = mdyFormat.parse(datevente);
			} catch (Exception e) {
				date = null;
			}
		}
		...
		if (!clt.isEmpty()) {
			if (date != null)
				fcts = metierFacture.getFacturesOfClient(numDossier, clt, date, pv, s);
			else
				fcts = metierFacture.getFacturesOfClient(numDossier, clt, pv, s);
		} else if (date != null)
			fcts = metierFacture.getFacturesClients(numDossier, date, pv, s);
		else
			fcts = metierFacture.getFacturesClients(numDossier, pv, s);

		double totalVentes = 0;
		for (Facture fct : fcts)
			totalVentes += fct.getTotal();
	...
	public String nouveaufacture(Model model, @RequestParam(name = "numCmd", defaultValue = "0") Long numCmd) {
		if (!model.containsAttribute("facture")) {
			...
			if (lcs == null)
				lcs = new HashMap<String, LigneFacture>();
			model.addAttribute("lfs", lcs.values());

			HashMap<String, ReductionFacture> redsf = (HashMap<String, ReductionFacture>) session.getAttribute("reds");
			if (redsf != null)
				model.addAttribute("redsf", redsf.values());

			HashMap<String, ReglementFacture> regsf = (HashMap<String, ReglementFacture>) session.getAttribute("regs");
			if (regsf != null)
				model.addAttribute("regsf", regsf.values());
		}
	...
	public String saveFacture(Model model, @RequestParam(name = "operation", defaultValue = "0") String operation,
		...
		HashMap<String, Object> res = validerFacture(model, operation, num, dateFactureString, codeClient,
				codeFournisseur);

		Date dL = null;
		SimpleDateFormat mdyFormat = new SimpleDateFormat("yyyy-MM-dd");
		try {
			dL = mdyFormat.parse(dateLivraison);
		} catch (Exception e) {
			res.put("10", "Date livraison invalide !");
		}
		...
		if (regs != null)
			for (ReglementFacture reg : regs.values()) {
		...
		if (reds != null)
			for (ReductionFacture red : reds.values()) {
	...
	public String editFacture(Model model, @RequestParam(name = "numero", defaultValue = "0") Long numero) {
		...
		if (!model.containsAttribute("facture")) {
			...
			facture = (Facture) model.asMap().get("facture");

		HashMap<String, ReductionFacture> redsf = new HashMap<>();
		HashMap<String, ReglementFacture> regsf = new HashMap<>();
		HashMap<String, LigneFacture> lfs = new HashMap<>();

		for (ReglementFacture rf : facture.getReglements())
			regsf.put("" + rf.getReglement().getId(), rf);
		for (ReductionFacture rd : facture.getReductions())
			redsf.put("" + rd.getReduction().getId(), rd);
		for (LigneFacture lf : facture.getLignesFacture())
			lfs.put(lf.getProduit().getRef(), lf);
	...
	HashMap<String, Object> validerFacture(Model model, String operation, Long numero, String date, String codeClient,
		...
		SimpleDateFormat mdyFormat = new SimpleDateFormat("yyyy-MM-dd");
		try {
			dateFact = mdyFormat.parse(date);
		} catch (Exception e) {
			res.put("3", "Date facture invalide !");
		}
	...
	public @ResponseBody HashMap storelf(@RequestParam(name = "refProduit", defaultValue = "0") String refProduit,
			@RequestParam(name = "qte", defaultValue = "0") Integer qte,
			@RequestParam(name = "prix", defaultValue = "0") Double prix) {
		HashMap<String, Object> res = new HashMap<>();
		if (refProduit.equals("0"))
			res.put("produit", "Selectionner d'abord le produit !");
		if (prix <= 0)
			res.put("prix", "Le prix doit etre un réel !");
		if (qte <= 0)
			res.put("qte", "La quantité doit etre au minimum 1 !");
		if (res.size() > 0)
			return res;
		...
		if (lfs == null)
			lfs = new HashMap<String, LigneFacture>();
	...
	public @ResponseBody HashMap storergf(@RequestParam(name = "reg", defaultValue = "0") Long reg,
			@RequestParam(name = "taux", defaultValue = "0") Double taux) {
		HashMap<String, Object> res = new HashMap<>();
		if (taux <= 0)
			res.put("taux", "Le taux doit etre un nombre positif !");
		if (reg == 0)
			res.put("reg", "Selectionner d'abord un reglement !");
		if (res.size() > 0)
			return res;
		...
		if (regs == null)
			regs = new HashMap<String, ReglementFacture>();
	...
	public @ResponseBody HashMap storerdf(@RequestParam(name = "red", defaultValue = "0") Long id,
			@RequestParam(name = "taux", defaultValue = "0") Double taux) {
		HashMap<String, Object> res = new HashMap<>();
		if (taux <= 0)
			res.put("taux", "Le taux doit etre un nombre positif !");
		if (id == 0)
			res.put("red", "Selectionner d'abord une reduction !");
		if (res.size() > 0)
			return res;
		...
		if (reds == null)
			reds = new HashMap<String, ReductionFacture>();
